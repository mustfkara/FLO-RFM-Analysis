...
For this purpose, the behaviors of the customers will be defined and groups will be formed
...
df_ = pd.read_csv("Projects/FLO_RFM_Analysis/flo_data_20k.csv")
df = df_.copy()
...
# Converting types of date-expressing variables from object to date
...
# Express the data preparation process as a function

def data_perp(dataframe):
    df["order_num_total"] = df["order_num_total_ever_online"] + df["order_num_total_ever_offline"]
    df["customer_value_total"] = df["customer_value_total_ever_offline"] + df["customer_value_total_ever_online"]
    date_columns = df.columns[df.columns.str.contains("date")]
    df[date_columns] = df[date_columns].apply(pd.to_datetime)
...
# What is the date of the last transaction in the dataset?
df["last_order_date"].max()   # 2021-05-30
# create analysis date by adding 2 days to the date
